...
(2) \textbf{Operational scale failure}. Scaling to larger networks, sequential PhaseNet on CPU requires on the order of 0.84~s per station at 250 TexNet stations and 0.23~s per station at 580 stations (Table~1), so full-window totals exceed real-time deadlines.
...
To establish a reference for the parallel strategies, we measured load time, merged-stream \texttt{annotate()}, and sequential per-station \texttt{classify()} for the four SeisBench-integrated models on CPU and GPU at 228, 250, and 580 stations (Table~1). EQCCT was excluded from this baseline because it uses a custom TensorFlow interface without a SeisBench-compatible stream pipeline. \textbf{Classify--per-station} is the mean wall time per station (total sequential classify time divided by $N$). All values are minima over five repeats with pre-copied streams. Table~2 reports CPU serial spot-checks at coarser station steps; those timings underpin the empirical serial curves in Figures~7--8.

\clearpage


Warm-cache model \textbf{load} times were consistent across architectures (about 1.16--1.24~s per Table~1). Offline batch inference via merged-stream \texttt{annotate()} remained the fastest single-call path at 228 stations (about 0.20--1.22~s depending on model and device). Per-station streaming via sequential \texttt{classify()} scales roughly linearly with $N$. At 228 stations, mean per-station CPU times span about 0.015--0.26~s (Table~1), i.e.\ about 1.4--60~s total classify time for the full window (CPU and GPU), consistent with Table~2; PhaseNet on CPU remains the heaviest SeisBench baseline in this study, while PhaseNetLight stays far lighter. GPU sequential means are substantially lower than CPU for the same $N$ in Table~1---from modest factors for some transformer variants to roughly an order of magnitude or more for PhaseNet (Table~1).

\subsection{Ripper: Ephemeral Task-Based Parallelism}

Across CPU Ripper runs (Table 4), total runtimes for 228 stations range from 34.3–76.1 s, while GPU Ripper configurations (best one- and two-GPU results per model, with Ray CPUs spanning 5–20) range from 52.5–127.4 s. The slowest GPU Ripper results occur for EQCCT, driven by repeated reinitialization of TensorFlow XLA-compiled graphs in each ephemeral task. In contrast, the PyTorch-based SeisBench models achieve more consistent CPU performance, typically 34–38 s. On GPU, these same models range from 52.5–67.6 s, depending on model and GPU count, with some configurations performing better on two GPUs than on one (Table 4).
