...
The Ripper method (Figure 2) uses a task-parallel strategy where each station is handled by an independent worker task scheduled by Ray (Moritz et al., 2018). Each task performs a full work cycle: loading the model into memory, performing inference, and unloading the model. Consequently, every station pays the full framework initialization cost (0.92–1.31 s). We used calibrated per-task memory budgets (Table 1) to prevent OOM errors. For GPU trials, Ripper requires higher VRAM per instance due to fragmentation and repeated CUDA context creation. As such, we applied scaling factors (1.7 for PhaseNet, 2.0 for EQCCT/EQTransformer) based on isolated testing.
...
The Model-Actor method (Figure 3) uses Ray actors to keep model instances persistently loaded in memory for the duration of the processing window. Each actor is initialized once and remains resident in RAM or VRAM while handling inference requests. Incoming waveforms are dispatched to these "hot" actors as a continuous stream. Actors are initialized in parallel to reduce additional serial processing costs to achieve maximum speedup (Amdahl et al., 1967). This architecture eliminates the redundant setup and teardown costs incurred by the Ripper method. While there is an initial pre-computation cost to load the requested actors, this is offset by the near-zero marginal cost of processing each additional station.
...
To establish a reference for the parallel strategies, we measured the two single-process inference methods for the four SeisBench-integrated models: offline batch method (a single `annotate()` call for all 228 stations) and per-station streaming method (228 sequential `classify()` calls). EQCCT was excluded from this baseline because it uses a custom TensorFlow interface without a SeisBench-compatible stream pipeline. All measurements were performed on a warm cache with five repeated runs; minimum times are reported (Table 2).
