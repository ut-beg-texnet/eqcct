...
def check_vram(self, model_vram_mb=3000):
    # We need to check that the requested VRAM does not exceed the capabilities of the system, if not, we need to exit safely before it happens
    avail_vram_list = []
    for gpu in self.selected_gpus: # selected_gpus is a list of GPU IDs 
        _, available_vram = get_gpu_vram(gpu_index=gpu)
        available_vram_mb = (available_vram * 1024) * 0.95 # Apply a 95% threshold to the available VRAM so we don't run the risk of overcommiting the GPU to what it may be capable for
        avail_vram_list.append(available_vram_mb)

    # We need to account for the number of model actors (one per GPU) we intended to run 
    # Remember that the model actor only needs to exist once per GPU, so we don't have to spawn intended_workers amount of raylets that have EQCCT instances inside of them
    # This will eat up the available memory very fast 
    num_model_actors = len(self.selected_gpus)
    # We also need to know how many workers we are intending to run in parallel 
    intended_workers = self.number_of_concurrent_station_predictions * self.number_of_concurrent_timechunk_predictions
    requesting_vram_mb = (intended_workers * self.vram_mb) + model_vram_mb # We calculate the VRAM requested

    for avail_vram in avail_vram_list: 
        if requesting_vram_mb > avail_vram: 
            self.logger.error(f"ERROR: Insufficient VRAM for GPU ID: {avail_vram_list.index(avail_vram)}!")
            self.logger.error(f"  Available (95%): {avail_vram:.0f} MB")
            self.logger.error(f"  EQCCT Model actors needed: {num_model_actors / num_model_actors:.0f}") # Should be 1 because we are only requesting 1 Model Actor per GPU
            self.logger.error(f"  EQCCT memory overhead (on VRAM): {model_vram_mb:.0f} MB")
            self.logger.error(f"  Workers need: {intended_workers:.0f} MB")
            self.logger.error(f"  Total needed: {requesting_vram_mb:.0f} MB")
            self.logger.error(f"  Please reduce the number of concurrent workers or vram_mb setting. Based on the current config, you will have OOMEs. Exiting....")
            exit()
