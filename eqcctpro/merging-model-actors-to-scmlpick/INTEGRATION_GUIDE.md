...
| `eqcctpro/functionality.py`    | `**RunEQCCTPro**` for batch- or trial-style runs with CSV output and configurable backends.                                                                                         |
...
For **EQCCT**, keep `**prepare_station_chunk`** windowing, overlap, and `**filterShift**` aligned with operational scmlpick unless you intentionally benchmark a change. Bandpass corners use `**_bandpass_hz_from_scmlpick_bindings**`: match the binding row whose `**key**` equals `**NET.STA**`, else fall back to the row whose key ends with the station code, else use `**default_band**` (typically **1–45 Hz**).

For **SeisBench**, the same `**prepare_station_chunk`** pipeline applies **your binding `BW(...)` corners** via an explicit `**default_band`** computed from `**df_filters**`, then `**_prepared_stream_to_seisbench_3c**` only chooses and renames the E/N/Z traces—no second independent filter story, and no hidden replacement of binding frequencies by an internal default when a binding row exists.
...
| SeisBench + 3C IO              | `eqcctpro/seisbench_models.py`                                       | `**SeisBenchModels**`, `**process_raw_station_stream_3c**`, `**mseed2stream_3c**`                                                                                                                                                                                                                                                             |
