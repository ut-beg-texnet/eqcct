...
| `eqcctpro/pick_output.py`      | Station file sink for batch picks: XML / ASCII / CSV (`**PickOutputSink**`, `**pick_output_format**`). Not used on the hot scmlpick `**scPhase**` path; relevant to `**RunEQCCTPro**`. |
| `eqcctpro/functionality.py`    | `**RunEQCCTPro**` / `**EvaluateSystem**` with `**waveform_filter_*`** and `**pick_output_format**`; batch outputs default to XML.                                                      |
...
For offline comparisons or benchmarks—not the hot real-time `**picker`** path—you should run `**RunEQCCTPro**` from `**functionality.py**` with the same preprocessing assumptions you care about, and diff picks or CSV outputs when you change scmlpick’s predictor or Ray wiring.
...
The reference tree now applies those frequencies through **`eqcctpro.waveform_filter`** (same semantics as **EQCCTPro** `**RunEQCCTPro**`): filter **type** (`bandpass`, `bandstop`, `lowpass`, `highpass`), **corners**, and **zerophase** are configurable as **`eqcct.waveformFilterType`**, **`eqcct.waveformFilterCorners`**, **`eqcct.waveformFilterZerophase`** in **`scmlpick.cfg`** / **`scmlpick.xml`**, and are forwarded into **`prepare_station_chunk`** for both EQCCT and SeisBench paths.
...
**Batch / offline:** `**RunEQCCTPro**` additionally exposes per-run `**pick_output_format**` (`xml`, `ascii`, `csv`) for `**X_prediction_results.***` files under each station output directory. Real-time scmlpick continues to emit picks through **`scPhase`** / SeisComP messaging, not those files.
...
| Waveform filter + batch output | `eqcctpro/waveform_filter.py`, `eqcctpro/pick_output.py`              | Align scmlpick `**prepare_station_chunk**` with `**apply_waveform_filter**`; batch XML/ASCII/CSV via `**pick_output_format**` only in `**RunEQCCTPro**`                                                                                                                                                                                          |
