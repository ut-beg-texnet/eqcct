...
- **Batch pick files** (**`RunEQCCTPro`** / **`EvaluateSystem`** only, via **`eqcctpro.pick_output`**): **`pick_output_format`** is **`xml`**, **`csv`**, or **`ascii`**. **`ascii`** writes a run-level **`summary_results.ascii`** (and **`summary_results_<chunk>.ascii`** when multiple timechunks), plus under **`<station>_outputs/`** the detail **`X_prediction_results.xml`** or **`.csv`** from **`ascii_station_pick_format`** and **`<station>.log`** (chronological P/S pick lines). With **`overwrite=False`**, skipped stations keep existing files and the ASCII summary is **merged** by station; **`overwrite=True`** removes the summary at job start and workers may replace per-station outputs. Real-time scmlpick still emits **`scPhase`** only.
