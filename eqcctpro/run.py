...
input_mseed_directory_path = '/home/skevofilaxc/workspace/clean_eqcct/eqcct/eqcctpro/230_stations_1_min_dt' # Change to local path 
output_pick_directory_path = '/home/skevofilaxc/workspace/clean_eqcct/eqcct/eqcctpro/outputs' # Change
log_file_path = '/home/skevofilaxc/workspace/clean_eqcct/eqcct/eqcctpro/outputs/eqcctpro.log' # Change
csv_filepath = '/home/skevofilaxc/workspace/clean_eqcct/eqcct/eqcctpro/csv/gpu' # Change
tmp_dir = '/lambda1a/skevofilaxc/tmp' # Change

# # EQCCTPro can run EQCCT or SeisBench models on a given input dir on either your GPU or CPU     
# eqcctpro_cpu_runner = RunEQCCTPro(use_gpu=False, # Defines if you use the GPU to run the model runner (bool)
#                                 model_type='eqcct', # Type of model to use: 'eqcct' or 'seisbench' (str)
...
#                                 p_model_filepath='/home/skevofilaxc/model/ModelPS/test_trainer_024.h5', # Filepath to where the P EQCCT detection model is stored (str). Only used if model_type='eqcct'
#                                 s_model_filepath='/home/skevofilaxc/model/ModelPS/test_trainer_021.h5', # Filepath to where the S EQCCT detection model is stored (str). Only used if model_type='eqcct'
#                                 seisbench_parent_model='PhaseNet', # SeisBench parent model name (str). Only used if model_type='seisbench'
#                                 seisbench_child_model='original', # SeisBench child model name (str). Only used if model_type='seisbench'
#                                 Detection_threshold=0.3, # Detection threshold for SeisBench models (float). Only used if model_type='seisbench'
#                                 number_of_concurrent_station_predictions=20, # Number of stations that are being analyzed in parallel (int)
#                                 number_of_concurrent_timechunk_predictions=1, # Number of timechunks that are being analyzed in parallel (int)
...
# # You can also uses SeisBench-based models. This example uses PhaseNet to perform P/S picking with the EQCCTPro workflow:
# eqcctpro_phasenet_runner = RunEQCCTPro(use_gpu=True,
#                                       model_type='seisbench',
#                                       seisbench_parent_model='PhaseNet',
#                                       seisbench_child_model='original',
#                                       P_threshold=0.3,        # Increased from default (e.g., 0.1)
#                                       S_threshold=0.3,        # Increased from default (e.g., 0.1)
#                                       Detection_threshold=0.3, # Used for models like EQTransformer
#                                       input_dir=input_mseed_directory_path,
#                                       output_dir=output_pick_directory_path,
#                                       log_filepath=log_file_path,
#                                       selected_gpus=[0, 1],
#                                       cpu_id_list=range(0,50),
#                                       vram_mb=2000,
...
# eqcctpro_phasenet_runner.run_eqcctpro() # Runs EQCCTPro on one instance of waveform data using GPU(s) as the main computing driver with SeisBench (PhaseNet) model
...
#                 output_dir=output_pick_directory_path, # Directory path to where the output picks and logs will be sent (str)
#                 log_filepath=log_file_path, # Filepath to where the EQCCTPro log will be written to and stored (str)
#                 csv_dir='/home/skevofilaxc/workspace/clean_eqcct/eqcct/eqcctpro/csv/gpu', # Directory path where the CSV's outputted by EvaluateSystem will be saved, doesn't need to exist, will be created if doesn't exist (str)
#                 P_threshold=0.001,  # Threshold in which the P probabilities above it will be considered as P arrival (float)
#                 S_threshold=0.02,   # Threshold in which the S probabilities above it will be considered as S arrival (float)
#                 p_model_filepath='/home/skevofilaxc/model/ModelPS/test_trainer_024.h5', # Filepath to where the P EQCCT detection model is stored (str). Only used if model_type='eqcct'
#                 s_model_filepath='/home/skevofilaxc/model/ModelPS/test_trainer_021.h5', # Filepath to where the S EQCCT detection model is stored (str). Only used if model_type='eqcct'
#                 seisbench_parent_model='PhaseNet', # SeisBench parent model name (str). Only used if model_type='seisbench'
#                 seisbench_child_model='original', # SeisBench child model name (str). Only used if model_type='seisbench'
#                 cpu_id_list=range(113,128), # Defines which specific CPU cores that sched_setaffinity will allocate for executing the current EQCCTPro process and is the maximum amount of cores EvaluteSystem can use in its trial iterations (list)
#                 min_cpu_amount=5, # Is the minimum amount of CPUs you want to start your trials with (int)
#                 cpu_test_step_size=5, # Is the desired step size for the trials will march from min_cpu_amount to len(cpu_id_list) (int)
#                 stations2use=100, # Controls the maximum amount of stations EvaluateSystem can use in its trial iterations (int)
...
#                 conc_station_tasks_step_size=1, # Is the concurrent station predictions step size you want each trial iteration to iterate with (int)
#                 start_time='2024-12-15 12:00:00', # The start time of the area of time that is being analyzed | Must follow the following convention YYYY-MO-DA HR:MI:SC (str)
#                 end_time='2024-12-15 12:01:00', # The end time of the area of time that is being analyzed | Must follow the following convention YYYY-MO-DA HR:MI:SC (str)
#                 conc_timechunk_tasks_step_size=1, # Is the concurrent timechunk predictions step size you want each trial iteration to iterate with
#                 timechunk_dt=1, # The length each time chunk is (in minutes) (int)
#                 waveform_overlap=0, # The duration (in minutes) for which each waveform overlaps with the others (int)
#                 tmp_dir=tmp_dir, # A temporary directory to store all temp files produced by EQCCTPro (str) | Used to help ease system cleanup and to not write to system's default temporary directory
#                 max_vram_mb=45000, # The maximum amount of VRAM all ModelActors can use (float); It reflects the upper limit of the iterative amount of VRAM we can test the ModelActors with | Must be a real value that is based on your hardware's physical memory space, if it exceeds the space the code will break due to OutOfMemoryError
#                 gpu_vram_safety_cap=0.95, # Safety cap for VRAM usage to help avoid OutOfMemory errors (float between 0 and 1)
#                 selected_gpus=[0, 1]) # List of GPU IDs on your computer you want to use (list) | Non-existing GPU IDs will cause the code to exit
