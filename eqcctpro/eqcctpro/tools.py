...
CANONICAL_CSV_HEADER = [
    ...
    "Model Used",
    # Memory Tracking Metrics (System-Wide RAM)
    "System Total RAM (MB)",
    "System Available RAM Before (MB)",
    "System Available RAM After (MB)",
    # Memory Tracking Metrics (Process/Raylet RAM)
    "Process RAM Baseline (MB)",
    "Process RAM After Run (MB)",
    "Process RAM Delta (MB)",
    "Process RAM Peak (MB)",
    # Memory Tracking Metrics (Ray Worker/Raylet RAM)
    "Num Raylet Processes",
    "Total Raylet RAM (MB)",
    "Avg Raylet RAM (MB)",
    "Max Raylet RAM (MB)",
    # Memory Tracking Metrics (GPU VRAM - only populated for GPU runs)
    "GPU Total VRAM (MB)",
    "GPU VRAM Free Before (MB)",
    "GPU VRAM Free After (MB)",
    "GPU VRAM Used by Trial (MB)",
    # Trial outcome
    "Trial Success",
    "Error Message",
]
...
def build_memory_trial_data(
    ...
    vram_before: Dict[str, float] = None,
    vram_after: Dict[str, float] = None,
) -> Dict[str, any]:
    """
    Build the memory-related trial data dictionary for CSV recording.
    
    Parameters:
    -----------
    mem_before : MemorySnapshot
        Memory snapshot taken before the trial.
    mem_after : MemorySnapshot
        Memory snapshot taken after the trial.
    vram_before : dict, optional
        VRAM snapshot before trial (for GPU runs).
    vram_after : dict, optional
        VRAM snapshot after trial (for GPU runs).
    
    Returns:
    --------
    dict
        Dictionary with all memory metrics for CSV columns.
    """
    delta = compute_memory_delta(mem_before, mem_after)
    
    trial_memory_data = {
        # System-wide RAM
        "System Total RAM (MB)": round(mem_before.system_total_mb, 2),
        "System Available RAM Before (MB)": round(mem_before.system_available_mb, 2),
        "System Available RAM After (MB)": round(mem_after.system_available_mb, 2),
        # Process RAM
        "Process RAM Baseline (MB)": round(mem_before.process_rss_mb, 2),
        "Process RAM After Run (MB)": round(mem_after.process_rss_mb, 2),
        "Process RAM Delta (MB)": round(delta["process_ram_delta_mb"], 2),
        "Process RAM Peak (MB)": round(max(mem_before.process_peak_mb, mem_after.process_peak_mb), 2),
        # Raylet RAM (use "after" snapshot as it captures Raylets during/after execution)
        "Num Raylet Processes": mem_after.num_raylet_processes,
        "Total Raylet RAM (MB)": round(mem_after.total_raylet_ram_mb, 2),
        "Avg Raylet RAM (MB)": round(mem_after.avg_raylet_ram_mb, 2),
        "Max Raylet RAM (MB)": round(mem_after.max_raylet_ram_mb, 2),
    }
    
    # GPU VRAM (only populated for GPU runs)
    if vram_before is not None and vram_after is not None:
        trial_memory_data["GPU Total VRAM (MB)"] = round(vram_before.get("gpu_total_vram_mb", 0), 2)
        trial_memory_data["GPU VRAM Free Before (MB)"] = round(vram_before.get("gpu_free_vram_mb", 0), 2)
        trial_memory_data["GPU VRAM Free After (MB)"] = round(vram_after.get("gpu_free_vram_mb", 0), 2)
        vram_used = vram_before.get("gpu_free_vram_mb", 0) - vram_after.get("gpu_free_vram_mb", 0)
        trial_memory_data["GPU VRAM Used by Trial (MB)"] = round(max(0, vram_used), 2)
    else:
        trial_memory_data["GPU Total VRAM (MB)"] = ""
        trial_memory_data["GPU VRAM Free Before (MB)"] = ""
        trial_memory_data["GPU VRAM Free After (MB)"] = ""
        trial_memory_data["GPU VRAM Used by Trial (MB)"] = ""
